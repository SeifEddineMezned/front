...
import { useState } from 'react' // Import kept for potential future state expansion
    ...
    return (
        <main className="relative w-full h-screen overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-10 pointer-events-none">
                ...
                <button
                    className="glass-panel px-6 py-3 rounded-xl pointer-events-auto text-cyan-200/70 hover:text-white hover:bg-white/5 transition-all text-sm font-mono tracking-widest uppercase border border-white/5 hover:border-white/20"
                    onClick={() => console.log('Supabase signOut placeholder')} // Placeholder: supabase.auth.signOut()
                >
                    Disconnect
                </button>
            </div>
            ...
            <div className="absolute bottom-8 left-0 w-full px-8 grid grid-cols-1 md:grid-cols-3 gap-6 pointer-events-none">
                ...
                <div className="glass-panel p-6 rounded-2xl flex flex-col gap-2 animate-scan border-l-4 border-l-cyan-500 opacity-80">
                    <h3 className="text-xs text-cyan-400 font-mono tracking-widest uppercase mb-1">System Status</h3>
                ...
                <div className="glass-panel p-6 rounded-2xl flex flex-col gap-2 opacity-80">
                    <h3 className="text-xs text-purple-400 font-mono tracking-widest uppercase mb-1">Active Nodes</h3>
                ...
                <div className="glass-panel p-6 rounded-2xl flex flex-col gap-2 opacity-80 md:col-start-3">
                    <h3 className="text-xs text-pink-400 font-mono tracking-widest uppercase mb-1">Recent Uplink</h3>
                    <div className="text-sm text-white/80 font-mono">
                        ID: <span className="text-white">USER_8821</span>
                    </div>
                    <div className="text-[10px] text-white/40 font-mono">
                        SYNC COMPLETED @ 14:02:44
                    </div>
                </div>
            </div>
