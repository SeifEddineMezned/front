...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          setError("damn the server is down");
          return;
        }
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden flex items-center justify-center bg-transparent">
      {/* 3D Scene Background */}
      ...
      <div className="relative z-10 w-full max-w-md p-6 animate-fade-in">
        {/* Glass Panel */}
        <div
          className="glass-panel rounded-3xl p-8 relative overflow-hidden transition-all duration-500 hover:border-cyan-500/30 bg-black/40 backdrop-blur-md border border-white/10"
          ...
          <form className="relative z-10 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="text-center mt-4">
              <button
                type="button"
                onClick={() => navigate("/")}
                className="text-xs text-white/50 hover:text-white transition-colors uppercase tracking-widest font-mono"
              >
                Return to Home
              </button>
            </div>
